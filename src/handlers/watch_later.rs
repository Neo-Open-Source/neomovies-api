...
use crate::{
    ...
    models::watch_later::{collection, ensure_indexes, WatchLater},
...
fn to_dto(f: WatchLater) -> WatchLaterDto {
    WatchLaterDto {
...
pub async fn handle_list(headers: &HeaderMap) -> VResp {
    ...
    let mut items: Vec<WatchLaterDto> = Vec::new();
    while let Ok(Some(fav)) = cursor.try_next().await {
        items.push(to_dto(fav));
    }
...
pub async fn handle_check(headers: &HeaderMap, kp_id_str: &str, media_type: &str) -> VResp {
    ...
    with_cors(success(json!({ "inWatchLater": exists })))
}
