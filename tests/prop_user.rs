...
use neowatch_api::models::user::{RefreshToken, User};
...
// Feature: neowatch-api-v2, Property 4: Profile update only modifies name and avatar
...
proptest! {
    ...
    fn prop_profile_update_only_modifies_name_and_avatar(
        ...
        let tokens: Vec<RefreshToken> = extra_tokens
            .iter()
            .take(extra_count)
            .map(|t| RefreshToken {
                token: t.clone(),
        ...
        prop_assert_eq!(&updated_user.role, &original_user.role, "role must not change");
...
// Feature: neowatch-api-v2, Property 5: Revoke-all empties refresh tokens
...
// Feature: neowatch-api-v2, Property 3: Refresh token rotation invalidates old token
